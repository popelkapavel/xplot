...
int not_zbuffered_wireframe = 1;
int wireframe_over_polygon = 0, wireframe_color = 8;
int polymodeflag = PYM_FILL; 
 ...
 */
static void dopoly(n)
     int n;
{
  int	i;
  if (!vdevice.clipoff)  polyclip(n);
  else 
    {
      nout = n;
      for (i = 0; i < n; i++) 
 ...
 */
static void polyoutline(n, ipx, ipy)
     int n;
     int ipx[], ipy[];
{
  int	i;
  ...
  if (n > 2)
    {
      for (i = 1; i < n; i++)
	{
	  (*vdevice.dev.Vdraw)(ip1[i], ip2[i]);
	  vdevice.cpVx = ip1[i];
	  vdevice.cpVy = ip2[i];
	}
      (*vdevice.dev.Vdraw)(ip1[0], ip2[0]);
  ...
  if(polymodeflag)
    {
      if(nout > 2) 
	{
	  if(not_zbuffered_wireframe) 
	    {
	      color(current_color_index); (*vdevice.dev.Vfill)(nout, ip1, ip2);
	      if(wireframe_over_polygon)  /* 7 is always BLACK */
		{ color(wireframe_color); my_polyoutline(nout, ip1, ip2);}
	    }
	  else 
	    {
	      color(0);  (*vdevice.dev.Vfill)(nout, ip1, ip2);
	      color(current_color_index);  my_polyoutline(nout, ip1, ip2);
	    }
	}
    }
  else
    {
      ...
      my_polyoutline(nout, ip1, ip2);
    }
}

/*********************************************************/
extern float *vts;  /* where are the vertices */
void my_poly(n,f)
     int n; float **f;
{
  int	i; float *t;
...
static int intersect(side, Ip, Pnt)
     int	side;
